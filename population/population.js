...
function showStats(statType, tableId) {
  fetch(url)
    ...
    .then((data) => {
      const worldData = data.filter((item) => item.countryname === "WORLD");
      if (worldData.length > 0 && statsMap[statType]) {
...
document.querySelectorAll(".clickable").forEach((item) => {
  item.addEventListener("click", (event) => {
    ...
    const [statType, tableId] = span.getAttribute("onclick");

    // Show the clicked table
    showStats(statType, tableId);
  });
});

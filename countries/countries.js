  ...
  fetch(url)
    ...
    .then((data) => {
      const countryData = data.filter(
        (item) =>
          item.countryname.toLowerCase() === selectedCountry.toLowerCase()
      );
      if (countryData.length > 0 && statsMap[statType]) {
...
function showStatsForCountry(country) {
  document.querySelectorAll(".table").forEach((table) => {
    table.style.display = "none";
  });
